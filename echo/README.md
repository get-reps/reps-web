...
Production success accepts only `https://www.getreps.io/login#token_hash=...&type=magiclink` or the same URL with `type=loginrecovery`, with no query, alternate credentials or extra fragment fields. Recovery permits existing-account verification without creating a new account. The existing login page stays untouched. Staging success requires `{connected:true,environment:"staging"}` and stays on a clear trial confirmation, without navigating or minting a production-app login. Trial saves remain separate from the real library. A production-shaped redirect in staging, or a trial result in production, is refused.
